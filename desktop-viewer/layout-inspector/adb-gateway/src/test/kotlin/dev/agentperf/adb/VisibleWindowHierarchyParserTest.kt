...
class VisibleWindowHierarchyParserTest {
    ...
    @Test
    fun `parses bare taskbar window as systemui navigation on Android 16`() {
        val windows = VisibleWindowHierarchyParser.parseWindows(
            zipBytes = EncodedHierarchyFixture.systemUiWithBareTaskbarZip(),
            packageName = "com.android.systemui",
        )
...
internal object EncodedHierarchyFixture {
    ...
    fun systemUiWithBareTaskbarZip(): ByteArray {
        val output = ByteArrayOutputStream()
        ZipOutputStream(output).use { zip ->
            zip.putNextEntry(ZipEntry("a3db2af StatusBar"))
            zip.write(hierarchy())
            zip.closeEntry()
            zip.putNextEntry(ZipEntry("b473c9d Taskbar"))
            zip.write(hierarchy())
            zip.closeEntry()
            zip.putNextEntry(ZipEntry("42177c9 com.codemx.anrdemo/com.codemx.anrdemo.MainActivity"))
            zip.write(hierarchy())
