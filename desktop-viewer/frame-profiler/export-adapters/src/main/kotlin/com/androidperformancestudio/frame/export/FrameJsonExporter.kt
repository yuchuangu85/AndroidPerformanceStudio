...
public class FrameJsonExporter {
    public fun export(
        ...
        Files.newBufferedWriter(output).use { writer ->
            ...
            writer.appendLine("    \"jankStatsClassifiedFrames\": ${result.summary.jankStatsClassifiedFrames},")
            writer.appendLine("    \"jankStatsJankFrames\": ${result.summary.jankStatsJankFrames},")
            writer.appendLine("    \"jankStatsJankRate\": ${result.summary.jankStatsJankRate.jsonValue()},")
            ...
            result.frames.forEachIndexed { index, frame ->
                ...
                writer.appendLine("      \"fragmentName\": ${sample.fragmentName.jsonValue()},")
                writer.appendLine("      \"pageName\": ${sample.pageName.jsonValue()},")
                writer.appendLine("      \"interactionState\": ${sample.interactionState.jsonValue()},")
                writer.appendLine("      \"renderThreadName\": ${sample.renderThreadName.jsonValue()},")
                writer.appendLine("      \"surfaceFlingerJankType\": ${sample.surfaceFlingerJankType.jsonValue()},")
                writer.appendLine("      \"jankStatsJank\": ${sample.jankStatsJank.jsonValue()},")
                writer.appendLine("      \"jankStatsReasons\": ${sample.jankStatsReasons.toList().sorted().jsonArray()},")
                writer.appendLine("      \"jankStatsRuleId\": ${sample.jankStatsRuleId.jsonValue()},")
                writer.appendLine("      \"jankStatsRuleVersion\": ${sample.jankStatsRuleVersion.jsonValue()},")
            ...
            result.attributions.forEachIndexed { index, attribution ->
                val renderThreadNames = attribution.renderThreadNames.toList().sorted()
                val surfaceFlingerJankTypes = attribution.surfaceFlingerJankTypes.toList().sorted()
                writer.appendLine("    {")
                ...
                writer.appendLine("      \"fragmentName\": ${attribution.fragmentName.jsonValue()},")
                writer.appendLine("      \"pageName\": ${attribution.pageName.jsonValue()},")
                writer.appendLine("      \"interactionState\": ${attribution.interactionState.jsonValue()},")
                writer.appendLine("      \"renderThreadNames\": ${renderThreadNames.jsonArray()},")
                writer.appendLine("      \"surfaceFlingerJankTypes\": ${surfaceFlingerJankTypes.jsonArray()},")
                writer.appendLine("      \"jankStatsFrames\": ${attribution.jankStatsFrames},")
