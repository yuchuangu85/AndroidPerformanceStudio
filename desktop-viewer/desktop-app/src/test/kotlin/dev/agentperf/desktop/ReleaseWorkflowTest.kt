...
class ReleaseWorkflowTest {
    ...
    fun `publish job has write access and stable native asset names`() {
        ...
        assertTrue(workflow.contains("AgentPerf-Inspector-${'$'}VERSION-windows-x64.exe"))
        assertTrue(workflow.contains("AgentPerf-Inspector-${'$'}VERSION-macos-x64.dmg"))
        assertTrue(workflow.contains("AgentPerf-Inspector-${'$'}VERSION-macos-x64.pkg"))
        assertTrue(workflow.contains("AgentPerf-Inspector-${'$'}VERSION-linux-x64.deb"))
        assertTrue(workflow.contains("AgentPerf-Inspector-${'$'}VERSION-linux-x64.rpm"))
    ...
    fun `workflow uses JDK 21 Gradle caching and official artifact actions`() {
        ...
        assertTrue(workflow.contains("uses: actions/download-artifact@v8"))
