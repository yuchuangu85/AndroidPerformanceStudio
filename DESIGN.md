...
## Winscope homepage macOS layout
- Status: Active
- Surface: `desktop-viewer/winscope/winscope-app` `WinscopeMainPage`
- Layout:
  - macOS-style top toolbar keeps device selection and capture actions in one control strip.
  - The body uses a padded canvas with a fixed capture sidebar and a rounded primary workspace card.
  - The analyzer workspace uses a compact segmented source switcher; timeline controls are grouped into title, transport/speed, jump, and scrubber rows.
  - Floating media remains layered over the analyzer workspace and is not moved into the sidebar or timeline.
- Visual language:
  - Reuse `LocalViewerColors`, `ViewerDimensions`, `MacOSTextButton`, `DropdownSelector`, and existing inline fields.
  - Use the existing macOS palette, hairline borders, 6–10 dp radii, restrained spacing, and horizontal scrolling for long source/preset lists.
- Interaction constraints:
  - Preserve capture-panel visibility toggle, session opening, source tab selection, timeline seeking, annotations, and floating video drag/resize behavior.
  - Keep the native Winscope handoff as the default-engine empty-state route.
- Responsive behavior:
  - Keep the capture sidebar fixed-width when visible; let the workspace consume remaining width.
  - Source tabs and preset/ProtoLog chips scroll horizontally instead of shrinking labels or changing their actions.
- Verification:
  - Run the Winscope app/core tests and the desktop distributable task after UI changes.
